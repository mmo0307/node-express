...
let UserService = class UserService {
    ...
    createUser({ name, email, password }) {
        return __awaiter(this, void 0, void 0, function* () {
            const newUser = new entity_1.UserEntity(email, name);
            const salt = this.configService.get('SALT');
            yield newUser.setPassword(password, Number(salt));
            const existedUser = yield this.userRepository.findByEmail(email);
    ...
    loginUser({ email, password }) {
        return __awaiter(this, void 0, void 0, function* () {
            const existedUser = yield this.userRepository.findByEmail(email);
            if (existedUser) {
                const user = new entity_1.UserEntity(existedUser.email, existedUser.name, existedUser.password);
